...
function parseSubDistricts2(){
    return new Promise (function (resolve, reject){
        // handle districts
        if (city.hasOwnProperty('districts')){
            // console.log('Districts: ' + city.districts.length);
            var districts = city['districts'];
            Promise.each(districts, function(district, index){
                if (district.hasOwnProperty('district')){//district belongs to another district
                    // console.log(index +'\t'+ district.name +'\t'+ district.district);
                    return processSubDistrict(district);
                }
            })
            .then (function(){
                resolve();
            ...
            // Promise.map(districts, function(district, index){
                // if (district.hasOwnProperty('district')){//district belongs to another district
                    // console.log(index +'\t'+ district.name +'\t'+ district.district);
                    // return processSubDistrict(district);
                // }
...
function processSubDistrict(district){
    return new Promise(function(resolve, reject){
...
Promise.resolve()
    ...
    .then(parseDistricts)
    .then(processPromises)
    .then(parseSubDistricts2)
    .then(parseBuildings2)
    .then(function(){
