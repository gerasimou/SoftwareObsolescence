	...
	d3.json("JSONFILE", function(root) {
	  ...
	  function display(d) {
		...
		g.append("rect")
			...
			.call(rect)
			.append("title")
			.text(function(d) { return d.name + " -test2- " + formatNumber(d.size); }); /*should be d.value*/
	  ...
	  function name(d) {
		return d.parent
			? "Department budget " + " - " + d.name// + " -  Cliquez ici pour dézoomer"
			: "Department's budget"//- Cliquez pour zoomer";
	  }
	  
	  function nameSave(d) {
		return d.parent
			? name(d.parent) + " - " + d.name + " -  (Click here to go back)"
			: d.name;
	  }
